...
import { CounterfeitCustomerEditData } from '../../../data/services/aml/Reports/Counterfeit/counterfeit-payload';


interface CounterfeitPayload {
    fromDate: string;
    toDate: string;
...
function CounterfeitCustomerEdit() {
    const { counterfeitId } = useParams<{ counterfeitId: string }>();
    ...
    const fetchAll = async () => {
        try {
            const response = await counterfeitApiService.counterfeitGetAll();
            setAll(response);
        } catch (error) {
    ...
    const counterfeitApiService = new CounterfeitApiService();
    const [searchPerformed, setSearchPerformed] = useState(false);
    const [data, setData] = useState<CounterfeitPayload[]>([]);
    ...
    const handleSearchInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const filteredData: CounterfeitPayload[] = all.filter(item =>
            ...
            counterfeitId: item.counterfeitId,
    ...
    const handleSearch = () => {
        ...
        counterfeitApiService
            .counterfeitGetCustomDate(startDateAsDate, endDateAsDate)
            .then((fetchedData: CounterfeitCustomerEditData[]) => {
                setSearchPerformed(true);
                const transformedData: CounterfeitPayload[] = fetchedData.map((entry) => {
                    return {
                        ...
                        counterfeitId: entry.counterfeitId,
                        uid: entry.uid,
                    };
                });
                ...
                const filteredData = transformedData.filter(
                    (item) =>
                        !hiddencomplaintIds.includes(item.counterfeitId.toString()) &&
    ...
    const handleTableRowClick = (uid: string, counterfeitId: string) => {
        navigate(`/CounterfeitView/${counterfeitId}/${uid}`);
    };

    useEffect(() => {
        const hiddencounterfeitIdsString = localStorage.getItem('hiddencounterfeitIds');
        const hiddencounterfeitIds = hiddencounterfeitIdsString ? JSON.parse(hiddencounterfeitIdsString) : [];
        const storedData = localStorage.getItem('customerData');
        if (storedData) {
            const transformedData = JSON.parse(storedData) as CounterfeitPayload[];
            const filteredData = transformedData.filter(
                (item) =>
                    item.counterfeitId.toString() !== counterfeitId &&
                    !hiddencounterfeitIds.includes(item.counterfeitId.toString())
            );
            setData(filteredData);
        }
    }, [counterfeitId]);
    ...
    const handleAllButtonClick = async () => {
        try {
            ...
            let response = await counterfeitApiService.counterfeitGetAll();
                                                    ...
                                                    <Table size="small" className="table report-table" style={{ width: '100%' }}>
                                                        ...
                                                        <TableBody>
                                                            {data
                                                                ...
                                                                .map((item: CounterfeitPayload, index: number) => (
                                                                    <TableRow key={index}>
...
export default CounterfeitCustomerEdit;
