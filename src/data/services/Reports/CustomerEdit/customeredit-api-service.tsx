...
class CustomerApiService {
    ...
    async getCustomDates(startDate: string, endDate: string): Promise<CustomerEditData[]> {
        try {
            const response = await this.httpClientWrapper.get(`api/v1/CustomerEdit?fromDate=${startDate}&toDate=${endDate}`);
    ...
    getQcViewDate = async (startDate: Date, endDate: Date, cmsId: string): Promise<QcViewDataPayload[]> => {
        try {
            const formattedStartDate = format(startDate, 'yyyy-MM-dd');
            const formattedEndDate = format(endDate, 'yyyy-MM-dd');
            const response = await this.httpClientWrapper.get(`api/v1/QcView?fromDate=${formattedStartDate}&toDate=${formattedEndDate}&cmsId=${cmsId}`);
            return response;
        } catch (error) {
            throw error;
        }
    ...
    async getManagerPending(startDate: Date, endDate: Date): Promise<QcPendingViewData[]> {
        try {
            const formattedStartDate = format(startDate, 'yyyy-MM-dd');
            const formattedEndDate = format(endDate, 'yyyy-MM-dd');
